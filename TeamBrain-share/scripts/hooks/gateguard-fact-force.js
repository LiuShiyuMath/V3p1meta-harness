#!/usr/bin/env node
/**
 * PreToolUse Hook: GateGuard Fact-Forcing Gate (TeamBrain-share)
 *
 * Vendored from everything-claude-code's
 ...
 *   - MultiEdit: same gate per file
 *   - Bash (destructive): list targets, rollback plan, quote instruction
 *   - Bash (routine): quote current instruction (once per session)
 *
 * Usage as a Claude Code PreToolUse hook (TeamBrain-share/hooks/hooks.json):
 *   {
 *     "type": "command",
 *     "command": "node /abs/path/to/TeamBrain-share/scripts/hooks/gateguard-fact-force.js",
 *     "timeout": 5
 *   }
 *
 * The hook reads the standard PreToolUse JSON payload from stdin and either
 * exits 0 silently (allow) or prints a hookSpecificOutput JSON document and
 * exits 0 (deny with reason — the JSON contract carries the deny decision).
 *
 * Repo of upstream package: https://github.com/zunoworks/gateguard
 */
...
// --- Core logic (exported for run-with-flags.js compatibility) ---
...
// --- CLI entry point: read PreToolUse JSON from stdin, emit decision ---
...
if (require.main === module) {
  ...
  process.stdin.on('end', () => {
    ...
    try {
      ...
      // On unexpected error, fail open (allow the tool call)
      process.exit(0);
    }
